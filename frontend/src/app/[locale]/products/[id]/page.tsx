...
export default function ProductDetailPage({ params }: { params: Promise<{ id: string; locale: string }> }) {
  ...
  const productsPath = locale && locale !== 'en' ? `/${locale}/products` : '/products';
  ...
  if (error || !product) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4">
        ...
        <Link href={productsPath} className="btn-primary">Browse Products</Link>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav className="mb-6" aria-label="Breadcrumb">
        <ol className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
          <li><Link href={homePath} className="hover:text-gray-900 dark:hover:text-white transition-colors">Home</Link></li>
          <li>/</li>
          <li><Link href={productsPath} className="hover:text-gray-900 dark:hover:text-white transition-colors">Products</Link></li>
          <li>/</li>
      ...
      <div className="lg:grid lg:grid-cols-2 lg:gap-12">
          ...
          {product.images.length > 1 && (
            <div className="flex gap-3 mt-4 overflow-x-auto pb-2">
              {product.images.map((img, i) => (
                ...
                >
                  <Image src={img.url} alt={img.alt || `View ${i + 1}`} fill unoptimized sizes="80px" className="object-cover" />
                </button>
              ))}
            </div>
          )}
        ...
        <div className="mt-8 lg:mt-0">
          {product.category && (
            <Link
              href={`${productsPath}?category=${product.category.slug}`}
